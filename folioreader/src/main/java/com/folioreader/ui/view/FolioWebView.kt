...
//            );
    ...
    fun initViewTextSelection() {
        ...
        _binding = TextSelectionBinding.inflate(LayoutInflater.from(ctw), this, false)
...
//        viewTextSelection = LayoutInflater.from(ctw).inflate(R.layout.text_selection, null)
        ...
        FolioReader.get()?.config?.let {
            if (!FolioReader.get().config.isEnableCopy) binding.copySelection.visibility =
                View.GONE
            binding.copySelection.setOnClickListener {
                dismissPopupWindow()
                loadUrl("javascript:onTextSelectionItemClicked(${it.id})")
            }
            if (!FolioReader.get().config.isEnableshare) binding.shareSelection.visibility =
                View.GONE
            binding.shareSelection.setOnClickListener {
                dismissPopupWindow()
                loadUrl("javascript:onTextSelectionItemClicked(${it.id})")
            }
            if (!FolioReader.get().config.isEnableDefine) binding.defineSelection.visibility =
                View.GONE
        }
